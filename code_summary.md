...
### 10) 폴더안의 파일 리스트 보기
```
$ dir ./test : 현재 폴더안의 test 폴더의 파일리스트를 출력해준다.
$ dir ../ : 상위 폴더의 파일리스트를 출력해준다.
$ dir ./test > list.txt : 현재 폴더안의 test 폴더의 파일리스트를 list.txt 로 만들어서 현재 폴더에 저장한다.
```
...
```
### 2) ubuntu server 실행 파일 만들기
```
$ touch ubuntu_start.sh : 실행파일
$ vim ubuntu_start.sh : 실행파일을 vim 편집기로 열기
$ 서버 실행 코드 입력 후 저장
$ ./ubuntu_start.sh : 현재 디렉토리안의 ubuntu_start.sh 파일을 실행
```
